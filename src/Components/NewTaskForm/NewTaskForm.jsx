...
const NewTaskForm = ({dataList, setTaskName, taskName, onSubmitTaskForm, closeTaskForm}) => {
    const [closeBtnFill, setCloseBtnFill] =useState('#345e37')
    return (
        <form onSubmit={e=>onSubmitTaskForm(e)} data-list={dataList} className={[boardStyles.task__form, commonStyles.blockIHidden].join(' ')}>
            ...
            <div className={boardStyles.btnContainer}>
                <button className={boardStyles.taskList__button}>+ add task</button>
                <svg width="14" height="14"
                     viewBox="0 0 14 14" fill="none"
                     xmlns="http://www.w3.org/2000/svg"
                     onMouseOver={()=>setCloseBtnFill('#2E8B57')}
                     onMouseOut={()=>setCloseBtnFill('#345e37')}
                     onClick={e => closeTaskForm(e)}
                >
                    <path
